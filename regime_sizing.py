...
def compute_position_size(proba_df: pd.DataFrame,
    ...
    # Days where regime 0 has the highest probability = "worst regime days"
...
def backtest_sized_strategy(prices: pd.Series,
    ...
    print("\n=== Strategy Comparison ===")
    print(f"{'Strategy':<30} {'Total Return':>14} {'Sharpe':>8} {'Avg Position':>14}")
    print("-" * 70)

    for name, ret, pos in [
        ("Buy & Hold",   daily_ret,  pd.Series(1.0, index=daily_ret.index)),
        ("Binary",       binary_ret, binary_pos),
        (sized_label,    sized_ret,  sized_pos),
    ]:
        total = (1 + ret).prod() - 1
        sharpe = _sharpe(ret)
        avg_pos = pos.mean()
        print(f"{name:<30} {total*100:>13.1f}% {sharpe:>8.2f} {avg_pos:>13.1%}")

    print(f"\nmin_position={min_position*100:.0f}%: you are always at least "
          f"{min_position*100:.0f}% invested, even in the worst detected regime.")
    print("Note: sized position uses 1-day lag (signal from yesterday's")
    print("close drives today's position) -- no lookahead.")
